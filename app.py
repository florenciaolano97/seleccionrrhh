...
def render_applications(user: dict) -> None:
    st.subheader("Candidatos, asignaciones y pipeline")
    ...
    with tab_pipeline:
        if not jobs:
            ...
            selected_country = l1.selectbox(
                "País",
                ["Todos"] + country_values,
                key="pipeline_country",
            )
            selected_province = l2.selectbox(
                "Provincia",
                ["Todas"] + province_values,
                key="pipeline_province",
            )
            selected_city = l3.selectbox(
                "Localidad",
                ["Todas"] + city_values,
                key="pipeline_city",
            )
            ...
            for row in rows:
                ...
                if (
                    ...
                    and row.get("province") != selected_province
                ):
                    continue
                if (
                    selected_city != "Todas"
    ...
    with tab_pool:
        ...
        if not pool_rows:
            st.info(
                "El banco todavía está vacío. Cargá CV o recibí postulaciones."
            )
        else:
            ...
            pool_province = p2.selectbox(
                "Provincia",
                ["Todas"] + provinces,
                key="pool_province",
            )
            pool_city = p3.selectbox(
                "Localidad",
                ["Todas"] + cities,
                key="pool_city",
            )

            pool_age_enabled = st.checkbox(
                "Filtrar banco por edad",
                key="pool_age_enabled",
            )
            pool_min_age, pool_max_age = 18, 80
            if pool_age_enabled:
                st.warning(AGE_FILTER_NOTICE)
                pool_min_age, pool_max_age = st.slider(
                    "Rango de edad del banco",
                    16,
                    90,
                    (18, 65),
                    key="pool_age_range",
            ...
            for row in pool_rows:
                ...
                if (
                    pool_province != "Todas"
                    and row.get("province") != pool_province
                ):
                    continue
                if (
                    pool_city != "Todas"
                ...
                candidate_item = dict(row)
                candidate_item["age"] = age
                filtered_pool.append(candidate_item)

            candidate_options = {
                (
                    f"#{row['id']} · {row['full_name']} · "
                    f"{row.get('city') or 'Sin localidad'}"
                ): row["id"]
            ...
            if jobs:
                ...
                if st.button(
                    "Asignar candidatos seleccionados",
                    type="primary",
                    ...
                    if not selected_candidates:
                        ...
                        st.success(
                            f"Asignaciones nuevas: {created_count}. "
                            f"Postulaciones existentes actualizadas: "
                            f"{updated_count}."
                        )
                        st.rerun()
            else:
                st.info(
                    "Creá una búsqueda para poder asignar candidatos."
                )
    ...
    with tab_detail:
        ...
        if not detail_rows:
            ...
            with h1:
                ...
                st.write(
                    f"**Ubicación:** "
                    f"{selected.get('country') or '—'} / "
                    f"{selected.get('province') or '—'} / "
                    f"{selected.get('city') or '—'}"
                )
                st.write(
                    f"**Etiquetas:** "
            ...
            if has_permission("manage_candidates"):
                with st.form(
                    ...
                    manual_reason = st.text_area(
                        "Razón de la decisión manual",
                        value=selected.get("manual_reason") or "",
                        help=(
                            "Es obligatoria cuando la decisión manual "
                            "difiere o reemplaza la recomendación automática."
                        ),
                    )
                    ...
                    st.markdown("**Datos administrativos y etiquetas**")
                    d1, d2 = st.columns(2)
            ...
            with p2:
                with st.expander("Formación", expanded=True):
                    st.write(
            ...
            if report:
                ...
                st.info(
                    "La postulación todavía no tiene análisis."
                )

            if selected.get("manual_recommendation"):
                st.markdown("#### Decisión manual registrada")
                st.write(
                    f"**Decisión:** "
                    f"{selected['manual_recommendation']}"
                )
                st.write(
                    f"**Razón:** "
                    f"{selected.get('manual_reason') or 'Sin detalle'}"
                )
                st.write(
                    f"**Notas internas:** "
                    f"{selected.get('recruiter_notes') or 'Sin notas'}"
                )
    ...
    with tab_criteria:
        if not has_permission("manage_jobs"):
            ...
            if st.button(
                "Cargar criterios sugeridos de la plantilla",
            ...
            with st.form(
                ...
                st.caption(
                    "Podés modificar términos, pesos y obligatoriedad. "
                    "Los filtros demográficos nunca se incorporan al puntaje."
                )
                ...
                st.markdown("**Umbrales de recomendación**")
                ...
                save_and_rescore = st.form_submit_button(
                    "Guardar criterios y recalcular candidatos",
                    type="primary",
                )
...
def render_company_portal(user: dict) -> None:
    ...
    if selected == "Inicio":
        ...
        st.info(
            "Módulo 4.1 activo: banco de candidatos, asignación a "
            "búsquedas, criterios editables, filtros y decisiones justificadas."
        )
