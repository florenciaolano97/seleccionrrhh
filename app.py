...
def init_db() -> None:
    ...
    # Módulo 3: campos ampliados de las búsquedas.
    for column_name, definition in {
        ...
        add_column_if_missing("jobs", column_name, definition)
                ...
                if not edited_values["title"].strip():
                    ...
                    try:
                        execute(
                            ...
                            """,
                            (
                                edited_values["title"].strip(),
                                edited_values["area"].strip(),
                                edited_values["seniority"],
                                edited_values["location"].strip(),
                                edited_values["work_mode"],
                                edited_values["contract_type"].strip(),
                                edited_values["description"].strip(),
                                edited_values["responsibilities"].strip(),
                                edited_values["must_have"].strip(),
                                edited_values["desirable"].strip(),
                                edited_values["competencies"].strip(),
                                edited_values["status"],
                                now_iso(),
                                selected_job["id"],
                                user["company_id"],
                            ),
                        ...
                        log_event(
                            ...
                            selected_job["id"],
                            {
                                "title": edited_values["title"],
                                "status": edited_values["status"],
                            },
                        )

                        st.success("Búsqueda actualizada.")
                        st.rerun()
                    except Exception as exc:
                        st.error(
                            f"No se pudo actualizar la búsqueda: {exc}"
                        )
...
def render_company_portal(user: dict) -> None:
    ...
    if selected == "Inicio":
        ...
        st.info(
            "Módulo 3 activo: importación desde Excel, PDF y Word, "
            "autocompletado editable y administración de búsquedas."
        )
            ...
            with st.container(border=True):
                ...
                with c2:
                    ...
                    if job.get("desirable"):
                        st.markdown("**Requisitos deseables**")
